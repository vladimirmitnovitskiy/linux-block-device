...
int main(int argc, char *argv[])
{
  ...
  printf("[WRITE] %zd bytes in %f sec (Speed: %.2f MB/s)\n", bytes_written,
         write_time, (bytes_written / 1024.0 / 1024.0) / write_time);
  ...
  printf("[READ] %zd bytes in %f sec (Speed: %.2f MB/s)\n", bytes_read,
         read_time, (bytes_read / 1024.0 / 1024.0) / read_time);

  // 3. DATA VERIFICATION
  if (memcmp(write_buf, read_buf, BUF_SIZE) == 0) {
    printf("SUCCESS: The data read is completely identical to the data "
           "written!\n");
  } else {
